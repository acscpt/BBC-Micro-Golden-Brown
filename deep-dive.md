...
Lines 120-130  Direct memory configuration of OS sound workspace
...
`VDU23;8202;0;0;0;` is a VDU 23 sequence that disables the flashing text cursor. `8202` in decimal = `&200A` - the high byte (`&20`) targets the 6845 CRTC cursor control register, and the value `&0A` with bit 5 set disables cursor display. Without this, the OS text cursor would blink independently over whatever row 0 column 0 happens to contain - visually interfering with the title screen mosaic graphics.
...
## Lines 120-130: Direct Memory Configuration
...
These lines reach directly into the BBC Micro OS **sound workspace** in RAM - the internal state tables the MOS maintains for its sound channel queue and envelope processing.

The `?` operator writes a single byte to an address. The `!` operator writes a 32-bit (4-byte) word.

| Statement | Address | Value | Effect |
| ----------- | --------- | ------- | -------- |
| `?&7DE0=&84` | `&7DE0` | `&84` | Channel 0 control byte |
| `?&7E30=&82` | `&7E30` | `&82` | Channel 1 control byte |
| `?&7E80=&81` | `&7E80` | `&81` | Channel 2 control byte |
| `!&7E08=&9D86` | `&7E08` | `&9D86` | Channel 1 control/timing word |
| `!&7E58=&9D86` | `&7E58` | `&9D86` | Channel 2 control/timing word |

The control byte values `&84`, `&82`, `&81` prime each channel's OS state so that SOUND commands are picked up immediately without the OS queue applying its own hold-off delays. The 32-bit word `&9D86` sets envelope/timing parameters in the channel state block so that the very short duration-1 notes used throughout the sequencer integrate correctly with the envelope processing interrupt.

This is a signature technique of expert BBC BASIC programming: use the public BASIC API for the normal path, but poke the OS workspace directly to override or pre-set internal state for tighter control. It is what separates this program from a naive implementation.
...
- Release: AR=255. Stored as a signed byte, 255 = **-1**: amplitude decreases by 1 per step. This is a **gradual ~350ms tail** from peak to silence, not an instantaneous cut.

The result is a **plucked** character: a steady rise to full volume over ~50ms, a brief hold, then a gentle fade - mimicking the sustain tail of a real harpsichord string.
...
Same structure, but `AA=127`. Attack rises at 127 per step - amplitude reaches ALA=128 in approximately **2 steps (20ms)** compared to 5 steps for Envelope 1. The amplitude 127 also maps to a significantly higher SN76489 volume register setting than amplitude 30, making the harmony voice considerably louder; its release tail is proportionally longer (~1.3 seconds). This pushes the inner harmony voice clearly forward in the mix - it strikes harder and sustains longer - complementing the softer lead without merging with it.

### Envelope Shape Comparison

The two envelopes produce quite different amplitude profiles. The tables below trace each over time, using SOUND `duration=1` (one envelope step ~ the full note-on duration before release is triggered):
...
Attack occupies almost the entire note duration. The release begins from amplitude 128 and takes ~350ms to reach silence.
...
Attack completes in ~20ms, leaving ~30ms of sustain at full amplitude before release. Because release starts from 128 rather than the Env 1 pattern's lower mid-attack value, the tail is nearly **4x longer**.
...
128 -+                    +----------------------------------------- ...-> 0
 ...
 32  |     /
     |    /
  0  ---------+---------+---------+---------+--------------------------
              10ms      20ms      30ms      50ms     (release both at -1/step)
              |<- - - - - - - Env 1 attack - - - - - ->|->  release ~350ms
              |<-Env 2 atk->|<- - - Env 2 sustain - - - ->|->  release ~1330ms
```

The SN76489 chip uses a 4-bit volume register where only the upper 4 bits of the amplitude value matter. Amplitude 128 maps to volume 8 (out of 15 = full volume), amplitude 30 maps to volume 1 - very quiet. Within a single step's duration, Envelope 2 is striking at nearly 8x the output power of Envelope 1. This difference in character is what sonically separates the harmony from the lead despite both using the same type of square-wave tone generator.
...
- **`Q A W S E D F T G Y H J I K O L P ; :`**: the QWERTY and ASDF rows in left-to-right **column pairs**  -  for each column, the QWERTY-row key followed by the ASDF-row key immediately below it: Q+A, W+S, E+D, T+G, Y+H, I+K, O+L, P+;. Two letters are absent: `R` and `U`. Their ASDF-row column partners `F` and `J` are still present and appear as lone entries at those positions, but the column-pair structure is otherwise unbroken throughout.
- **`[ ]`**: the rightward extension of the QWERTY row.
- **`_ 1 2 3 4 5 6 7`**: the number row, highest pitches.

Why `R` and `U` specifically are excluded is not evident from the code  -  34 notes was sufficient for the arrangement, and their absence leaves the column-pair geometry otherwise clean.
...
The `SOUND` duration is always `1` - one 5ms OS tick. Notes are never held by the duration parameter. The audible length comes entirely from the ENVELOPE shape: the pluck-and-decay profile described above. This avoids any OS queue buildup from overlapping durations.
...
&7C50      2
  ...
&7E80     16   <- Piano keyboard row (drawn by line 110)
&7ED0     18   <- Voice 1 cursor row  (offset -80 from row 16)
&7F20     20   <- Voice 2 cursor row  (offset -160 from row 16)
  ...
&7FC8     24   Bottom of screen
```

Each row is 40 bytes - one byte per character cell. `&7E80 - &7C00 = 640 = 16 x 40`, confirming row 16.

### The Two Teletext Bytes Used for the Cursor

The cursor is drawn using two specific Teletext control codes:

- **Byte 157 (`&9D`) = New Background**: instructs the SAA5050 to set the background colour of subsequent cells to the current foreground colour. Since the PRINT statements earlier set up a white foreground context on this row, `&9D` makes the cell's background white - a visible bright block.
- **Byte 156 (`&9C`) = Black Background**: resets the background to black. Written to the cell immediately after the cursor, it prevents the white background from continuing rightward across the rest of the row.
...
2. **Line 360: Compute new cursor position.**
   ...
   - `OV%(J) - 60`: maps the note index (1-34 scale) to a horizontal column offset, positioning the cursor above the corresponding key in the piano diagram.
   - `-80*J`: 80 bytes = 2 rows x 40 bytes/row. Voice 0 -> row 16, Voice 1 -> row 18, Voice 2 -> row 20. Three simultaneous cursors stacked vertically, one per voice.
...
4. **Line 380: Draw cursor.** `?Y%(J) = 157 : Y%(J)?1 = 156` - New Background at the cursor position (creates white block), Black Background at the next cell (stops it spreading right).
...
`REPEAT UNTIL TIME >= T` is a **busy-wait**: BASIC spins checking `TIME` until it reaches the pre-recorded target. Then `NEXT` advances `K`. When the `FOR K` loop completes (all characters in the current phrase played), `GOTO 170` returns to the section dispatcher to start the next phrase.
...
The first half of all three strings is identical to section 390. The second half diverges  -  a variation matching the natural chorus repeat structure of the original recording.
...
2. **Envelope differentiation gives timbral separation** - Envelope 1 (slow 50ms attack, long 350ms release) for lead and bass; Envelope 2 (fast 20ms attack, long sustain, 1330ms release) for harmony. The two envelope characters push the voices apart in time even when they share pitches. The harmony rings on after the lead note dies.
...
7. **The OS is used, then bypassed, then worked around** - `ENVELOPE` and `SOUND` for the public API; direct pokes to `&7DE0` and `&7E80` to configure beneath it; `?` and `!` on screen RAM addresses to avoid the VDU driver entirely. Three levels of system access, all deployed where appropriate.
